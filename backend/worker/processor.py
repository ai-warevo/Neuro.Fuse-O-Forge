...
class TaskProcessor:
    ...
    async def execute(self, message_id: str, fields: dict):
        ...
        try:
            ...
            await self.broker.publish_result_to_stream(task_id, TaskStatus.SUCCESS, output_path)
            self.logger.info(f"✅ Готово: {task_id}")
        except Exception as e:
            self.logger.error(f"❌ Ошибка пайплайна {task_id}: {str(e)}")
            await self.broker.publish_result_to_stream(task_id, TaskStatus.ERROR, None, str(e))